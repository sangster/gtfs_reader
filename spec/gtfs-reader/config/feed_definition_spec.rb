...
describe GtfsReader::Config::FeedDefinition do
  ...
  it { expect(feed.optional_files.map(&:name)).to eq %i[routes trips] }
  ...
  context 'with a file defined' do
    ...
    it {
      expect(feed.file(:agency).required_columns.collect(&:name))
        .to eq required_names
    }
    it {
      expect(feed.file(:agency).optional_columns.collect(&:name))
        .to eq optional_names
    }
    it {
      expect(feed.file(:agency).unique_columns.collect(&:name))
        .to eq unique_names
    }
  end
end
